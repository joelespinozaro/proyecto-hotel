                    ...
                    <Row>
                        {
                            habitacion.map((item, index) => {
                                ...
                                return (
                                    <Col key={index} md={4} className={`mt-5 ${!item.estado ? "disabled" : ""}`} style={{display:'flex', justifyContent:'center'}}>
                                        <Card style={{ width: '18rem' }}>
                                            <Card.Img variant="top" src={getImage(item.tipoHabitacion)} />
                                            <Card.Body>
                                                <Card.Title>Habitacion Nro {item.numHabitacion}</Card.Title>
                                                <Card.Text className="font-weight-bold" style={{ fontSize: 12 }}>
                                                    Tipo : {item.tipoHabitacion} <br></br>
                                                    S/ Precio : {item.precio}<br></br>
                                                    <i className="text-justify font-weight-normal">
                                                        {getDescription(item.tipoHabitacion)}
                                                    </i>
                                                </Card.Text>

                                                <Button variant="primary" href={`/reservas/${item.id}`} >Generar Reserva</Button>
                                            </Card.Body>
                                        </Card>
                                    </Col>
