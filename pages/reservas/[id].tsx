...
export default function GenerarReservaPage({ habitacion, clientes, reserva }) {
  ...
  const [idRecepcionista, setIdRecepcionista] = useState(
    "9ff245e5-565f-4022-892d-615385fa9747"
  );
  const [fechaInicio, setFechaInicio] = useState(
    reserva?.fechaInicio || dateTransform()
  );
  const [fechaFin, setFechaFin] = useState(
    reserva?.fechaFin || dateTransform()
  );

  const generarReserva = async () => {
    const recepcionistaId = JSON.parse(
      localStorage.getItem("recepcionista")
    ).id;
    try {
      const body = {
        idCliente: idCliente,
      ...
      const result = await fetch(
        `/api/reservas/${reserva ? "update" : "new"}`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      <Row>
        ...
        >
          <Card style={{ width: "18rem" }}>
            <Card.Img
              variant="top"
              src={getImage(habitacion?.tipoHabitacion)}
            />
            <Card.Body>
              <Card.Title>
                Habitacion Nro {habitacion?.numHabitacion}
              </Card.Title>
              <Card.Text className="font-weight-bold" style={{ fontSize: 12 }}>
                Tipo : {habitacion?.tipoHabitacion} <br></br>
                S/ Precio : {habitacion?.precio}
                <br></br>
              ...
              >
                ...
                {clientes.map((client, index) => (
                  <option
                    key={index}
                    value={client.id}
                  >{`${client.nombres}  ${client.apellidos}`}</option>
                ))}
              </select>
            </Col>
          </Row>
